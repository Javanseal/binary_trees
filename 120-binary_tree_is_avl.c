...
/**
 * binary_tree_height - AFunction that measures the height of a binary tree
 ...
 * @high: a node that point to the higher node selected
 * @lower: a node that point to the lower node selected.
...
/**
 * binary_tree_is_avl - to checks if a binary tree is a valid AVL tree.
 * @tree: a node that point to the tree to check.
 * Return: 1 if tree is AVL, 0 if not.
 */
